...
-- _Cette technique d'attaque fonctionne en incluant du code malicieux ou un lien dans une page qui accède à une application web dont l'utilisateur est réputé avoir accès. Si la session pour cette application web n'a pas expirée, l'attaquant peut exécuter des commandes non autorisées._

!images/csrf.png!

Dans le <a href="#sessions">chapitre sur les sessions</a>, vous avez appris que la plupart des applications Rails utilisent des sessions à base de cookies. Soit elles stockent un identifiant de session dans le cookie et les données coté serveur, soit l'intégralité des données de session est présente coté client. Dans les deux cas, le navigateur enverra automatiquement le cookie à chaque requête vers le domaine concerné, s'il peut trouver un cookie pour ce domaine. Le point controversé est qu'il enverra également le cookie si la requête part d'un site sur un autre domaine. Mais commençons avec un exemple :

* Bob visite un forum et voit un message posté par un hacker où un des éléments HTML, une image, a été forgé. Cet élément fait référence à une commande sur l'application de gestion de projets de Bob; plutôt qu'à un fichier d'image.
* +&lt;img src="http://www.webapp.com/project/1/destroy"&gt;+
* La session de Bob sur www.webapp.com est encore active, parce qu'il ne sait pas déconnecter quelques minutes auparavant.
* En affichant le message, le navigateur trouve la balise image. Il essaye de la charger depuis www.webapp.com. Comme expliqué auparavant, il envoie aussi dans la requête le cookie avec l'identifiant de session valide.
* L'application web sur www.webapp.com vérifie les informations de l'utilisateur dans la session correspondant et détruit le projet avec l'identifiant 1. Puis elle renvoie une page de résultat qui ne correspond pas aux attentes du navigateur, du coup il n'essaye pas d'afficher l'image.
* Bob n'a pas remarqué l'attaque -- mais quelques jours plus tard, il découvre que le projet n°1 a disparu.

Il est important de noter que l'image ou lien forgé n'a pas nécessairement besoin d'être placé sur le domaine de l'application web, il peut être n'importe où – dans un forum, un billet de blog ou un courriel.
...
h3. Redirections et Fichiers

Une autre classe de vulnérabilités liés à la sécurité entoure l'utilisation des redirections et des fichiers dans les applications web.

h4. Les redirections

-- _Les redirections dans une application web sont un outil sous-estimé par les crackers : non seulement l'attaquant peut renvoyer l'utilisateur vers un site piégé, mais il peut aussi créé une attaque sur l'application elle-même._

À chaque fois que l'utilisateur est autorisé à passer (une partie de) l'URL de redirection, c'est une vulnérabilité potentielle. L'attaque la plus évident serait de rediriger l'utilisateur vers une fausse application web qui ressemble et se comporte exactement comme l'originale. Cette attaque dite de phishing fonctionne en envoyant un lien qui ne paraît pas suspicieux dans un email ) des utilisateurs, en injectant le lien par une attaque XSS dans l'application web ou encore en déposant de lien sur un site externe. Le lien n'inspire pas la suspicion car il commence avec l'URL de l'application web originale, l'URL du site de l'attaquant n'apparaît que caché dans le paramètre de redirection :  http://www.example.com/site/redirect?to= www.attacker.com. Voici un exemple d'une action legacy :
...
Cela redirigera l'utilisateur vers l'action main s'il essaye d'accéder à l'action legacy. L'intention du développeur était de préserver les paramètres de l'URL de l'action legacy et les transmettre à l'action main. Pourtant, un attaquant peut l'exploiter à son compte en incluant une clé host dans l'URL :

<plain>
http://www.example.com/site/legacy?param1=xy&param2=23&host=www.attacker.com
</plain>

En fin d'URL, cette clé ne sera pas remarquée et redirigera l'utilisateur vers le domaine attacker.com. Une simple protection est de _(highlight)n'inclure que les paramètres attendus dans l'action legacy_ (une approche par liste blanche, par opposition à la suppression des paramètres non-voulus). _(highlight) Et si vous redirigez vers une URL, vérifiez-la avec une liste blanche ou une expression rationnelle_.

h5. Attaque XSS autonome

L'utilisation du protocole data permet une autre attaque XSS autonome par redirection qui fonctionne dans Firefox et Opera. Ce protocole affiche son contenu directement dans le navigateur et peut couvrir n'importe quoi depuis du HTML ou Javascript aux images :

+data:text/html;base64,PHNjcmlwdD5hbGVydCgnWFNTJyk8L3NjcmlwdD4K+

Cet exemple est un Javascript encodé en Base64 qui affiche une simple boîte de message. Dans une URL de redirection, l'attaquant peut rediriger vers cette URL avec le code malicieux en son sein. Pour contrer cela, _(highlight)n'autorisez pas les utilisateurs à fournir (une partie de) l'URL de redirection_.

h4. Uploads de fichiers

-- _Soyez sûr de ne pas écraser d'importants fichiers avec les uploads de fichiers, et traitez les médias de manière asynchrone._

De nombreuses applications web autorisent leurs utilisateurs à uploader des fichiers. _(highlight)Les noms de fichiers, que l'utilisateur peut choisir (partiellement), devraient toujours être filtrés_ car un attaquant peut utiliser un nom de fichier malicieux pour écraser un fichier sur le serveur. Si les fichiers uploadés sont stockés dans /var/www/uploads et que l'utilisateur entre un nom de fichier comme “../../../etc/passwd”, il peut écraser un fichier important. Bien sûr, l'interpréteur Ruby aurait besoin des permissions appropriées pour faire cela – une autre bonne raison de faire tourner les serveurs web, les bases de données et les autres programmes avec un utilisateur UNIX avec des droits restreints.

Pour filtrer les noms de fichier entrés par les utilisateurs, _(highlight)n'essayez pas d'enlever les parties dangereuses_. Pensez à un cas où l'application web supprime tous les “../” dans le nom d'un fichier et que l'attaquant utilise une chaîne telle que “....//” - le résultat sera “../”. Il vaut mieux utiliser une approcher par liste blanche, qui _(highlight)vérifiera la validité d'un nom de fichier avec un ensemble de caractères autorisés_. C'est à opposer à l'approche par liste noire qui consiste à supprimer les caractères interdits. Si jamais le nom de fichier n'est pas valide, refusez-le (ou remplacez les caractères pas acceptés), mais ne les enlevez pas. À titre d'exemple, voici le nettoyeur de nom de fichier utilisé dans le "plugin attachment_fu":http://github.com/technoweenie/attachment_fu/tree/master :

<ruby>
def sanitize_filename(filename)
  filename.strip.tap do |name|
    # NOTE: File.basename ne fonctionne pas correctement sur Unix avec les chemins Windows
    # Prend seulement le nom de fichier, pas tout le chemin
    name.sub! /\A.*(\\|\/)/, ''
	# Finalement, remplacer tous les caractères qui ne sont pas alphanumériques,
	# un tiret ou un point par des soulignés
    name.gsub! /[^\w\.\-]/, '_'
  end
end
</ruby>

Un inconvénient significatif du traitement synchrone des fichiers uploadés (comme peut le faire le plugin attachment_fu avec les images) est sa _(highlight)vulnérabilité aux attaques par déni de service_. Un attaquant peut lancer l'uploads d'images depuis plusieurs ordinateurs en parallèle, ce qui va augmenter la charge du serveur et pourrait éventuellement faire planter le serveur ou le rendre indisponible.

La solution la plus efficace à cela est de _(highlight)traiter de manière asynchrone les médias_ : enregistrer le fichier et programmez une requête de traitement dans la base de données. Un second processus se chargera du traitement du fichier en tâche de fond.

h4. Code exécutable dans les fichiers uploadés

-- _Le code source d'un fichier uploadé peut être exécuté si celui-ci est placé dans certains répertoires spécifiques. Ne placez pas les fichiers uploadés dans le répertoire /public de Rails si c'est le répertoire racine d'Apache._

Le célèbre serveur web Apache a une option appelée DocumentRoot. Elle sert à définir le répertoire racine de chaque site web, et tout ce qui se trouve sous ce répertoire pourra être traité par le serveur web. S'il s'y trouve des fichiers avec une certaine extension, le code de ceux-ci pourra être exécuté quand ils seront demandés (cela peut demander que certaines options soient définies). Les fichiers PHP et CGI en sont des exemples. Maintenant, pensez à la situation où un attaquant upload un fichier “file.cgi” avec du code dedans, celui-ci sera exécuté quand quelqu'un téléchargera ce fichier.

_(highlight)Si le DocumentRoot d'Apache pointe sur le répertoire /public de Rails, ne placez pas les fichiers uploadés dedans_, enregistrez-les au moins un niveau plus bas.

h4. Téléchargement de fichiers

-- _Assurez-vous que les utilisateurs ne peuvent pas télécharger de fichiers arbitraires._

De la même manière que vous devez filtrer les noms de fichier pour les uploads, vous devez les filtrer pour les téléchargements. La méthode send_file() envoie les fichiers depuis le serveur vers le client. Si vous utilisez un nom de fichier entré par un utilisateur sans le filtrer, n'importe quel fichier peut être téléchargé :

<ruby>
send_file('/var/www/uploads/' + params[:filename])
</ruby>

Passez simplement un nom de fichier comme “../../../etc/passwd” pour télécharger les informations de login du serveur. Une solution simple contre cela est de _(highlight)vérifier que le fichier demandé est bien dans le répertoire attendu_ :
...
Une autre approche (supplémentaire) est d'enregistrer les noms de fichier dans la base de données et de nommer les fichiers sur le disque d'après les identifiants dans la base de données. C'est aussi une bonne approche pour éviter qu'un éventuel code dans un fichier uploadé puisse être exécuté. Le plugin attachment_fu procède de cette manière

h3. Sécurité des Intranet et Administration

-- _Les intranets et les interfaces d'administration sont des cibles populaires pour les attaques car ils autorisent des accès privilégiés. Bien que cela demanderait des mesures de sécurité supplémentaires, c'est le contraire qui se passe dans le monde réel._

En 2007 est apparu le premier cheval de Troyes fait sur mesure pour voler des informations depuis un intranet, en l'occurrence, le site web "Monster pour les recruteurs" de Monster/com, une application web pour l'emploi en ligne. Les chevaux de Troyes faits sur mesure sont très rares, jusqu'ici, et le risque reste faible, mais c'est une possibilité et un exemple qui montre que la sécurité du poste client est aussi importante. Pourtant, les plus grandes menaces pour les intranets et interfaces d'administration sont les attaques XSS et CSRF.

*XSS*  Si votre application réaffiche des entrées malicieuses de l'utilisateur depuis l'extranet, l'application est vulnérable au XSS. Les noms d'utilisateurs, commentaires, rapports de spam et adresses de livraison en sont quelques exemples.

Il suffit d'un seul endroit dans l'interface d'administration ou l'intranet où les données ne sont pas nettoyées pour rendre vulnérable l'application dans sa globalité. Les exploits possibles vont du vol du cookie d'un administrateur privilégié à l'injection d'une iframe pour voler le mot de passe d'un administrateur, en passant par l'installation d'un logiciel malicieux à travers une faille de sécurité d'un navigateur pour prendre le contrôle de l'ordinateur d'un administrateur.

Veuillez vous référer à la section sur les injections pour les protections contre les XSS. Il est _(highlight)recommandé d'utiliser le plugin SafeErb_ pour les intranets et interfaces d'administration.

*CSRF*  Cross-Site Request Forgery (CSRF) est une méthode d'attaque géante, elle permet à l'attaquant de faire tout ce qu'un administrateur ou un utilisateur de l'intranet pourrait faire. Comme nous avons déjà vu comment CSRF fonctionne, voici quelques exemples appliqués aux intranets et interfaces d'administration.

Un exemple du monde réel est "la reconfiguration de routeur par CSRF":http://www.h-online.com/security/Symantec-reports-first-active-attack-on-a-DSL-router--/news/102352. Les attaquants ont envoyé un courriel malicieux, avec du CSRF dedans, à des utilisateurs mexicains. Ce courriel affirme qu'une e-card les attend, mais il contient aussi une balise image qui provoque une requête HTTP de type GET pour reconfigurer le routeur de l'utilisateur (qui est un modèle très populaire à Mexico). La requête change la configuration DNS de telle sorte que les accès un site bancaire basé à Mexico soient transférés sur le site de l'attaquant. Tous ceux qui visitent le site bancaire à travers le routeur voient le faux site web de l'attaquant et se font voler leurs identifiants.

Un autre exemple a modifié l'adresse de courriel et le mot de passe sur Google Adsense. Si la victime était connecté sur Google Adsense, l'interface d'administration des campagnes de publicité de Google, un attaquant peut changer ses identifiants.

Une attaque populaire est d'envoyer du spam sur votre application web, votre blog ou forum pour propager un XSS. Bien sûr, l'attaquant a besoin de connaître la structure des URL, mais la plupart des URL générés par Rails sont relativement directes ou seront faciles à trouver si c'est l'interface d'administration d'une application Open Source. L'attaquant peut même tenter des milliers d'essais juste en incluant des balises IMG malicieuses pour chaque combinaison possible.

Pour _(highlight)se protéger contre les CSRF dans les interfaces d'administration et les applications Intranet, veuillez vous référer à la section sur les contremesures de CSRF_.
